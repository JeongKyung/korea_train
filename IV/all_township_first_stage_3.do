...
eststo: quietly ivregress 2sls Lmean Lcell_num town_eup (dist_ktx=`x') ,vce(cluster sigungu_cd) // (1)
estadd local FE ""
eststo: quietly ivregress 2sls Lmean Lcell_num town_eup post2004 dist_post (dist_ktx=`x'),vce(cluster sigungu_cd) // (2)
estadd local FE ""
eststo: quietly ivregress 2sls Lmean Lcell_num town_eup post2004 dist_post county* (dist_ktx=`x'),vce(cluster sigungu_cd) // (3)
estadd local FE "X"
eststo: quietly ivregress 2sls Lmean Lcell_num town_eup post2004 dist_post county* (dist_ktx=`x') if big_city==0, vce(cluster sigungu_cd) // (4)
estadd local FE "X"
esttab using ./two_stage_`x'_3.tex, replace ///
nomtitles mgroups("Light Mean", pattern(1 0 0 0) ///
span prefix(\multicolumn{@span}{c}{) suffix(})) ///
keep(dist_ktx Lcell_num town_eup post2004 dist_post) se(3) label title(Two-Stage Results using `y' as the instrument) ///
